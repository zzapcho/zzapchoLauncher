...
use mc_launcher_core::prelude::{
    ...
    LoaderVersion, ProgressEvent,
};
use mc_launcher_core::{
    core::{maven::MavenCoordinate, rules::{evaluate_rules, FeatureSet}, version::VersionJson},
    platform::Platform,
};
...
fn loader_spec(loader: &str, version: &str) -> Result<Option<LoaderSpec>, String> {
    let version = if version.trim().is_empty() { LoaderVersion::LatestStable } else { LoaderVersion::Exact(version.to_string()) };
    match loader {
        "vanilla" => Ok(None),
...
fn install_fallback_maven_libraries(app: &tauri::AppHandle, minecraft_dir: &Path, version: &VersionJson) -> Result<(), String> {
    let client = reqwest::blocking::Client::builder()
        .user_agent("zzapchoLauncher/0.1.0")
        .build().map_err(|error| error.to_string())?;
    for library in &version.libraries {
        if !evaluate_rules(&library.rules, Platform::current(), &FeatureSet::default()) { continue; }
        if library.downloads.as_ref().and_then(|downloads| downloads.artifact.as_ref()).is_some() || library.natives.is_some() { continue; }
        let Some(repository) = library.url.as_deref() else { continue };
        let coordinate = MavenCoordinate::parse(&library.name).map_err(|error| error.to_string())?;
        ...
        let url = format!("{}/{}", repository.trim_end_matches('/'), relative.to_string_lossy().replace('\\', "/"));
        ...
        response.copy_to(&mut output).map_err(|error| error.to_string())?;
    }
    Ok(())
}
...
fn sync_content(profile_dir: &Path, content: &[ContentToggle]) -> Result<(), String> {
    ...
    for item in content {
        if !valid_kinds.contains(item.kind.as_str()) { continue; }
        let folder = match item.kind.as_str() {
            "resourcePacks" => "resourcepacks",
            "shaders" => "shaderpacks",
            _ => "mods",
        };
        let Some(file_name) = Path::new(&item.file_name).file_name() else { continue };
...
fn prepare_and_launch(app: tauri::AppHandle, request: LaunchMinecraftRequest) -> Result<LaunchStarted, String> {
    let app_data = app.path().app_data_dir().map_err(|error| error.to_string())?;
    let minecraft_dir = app_data.join("minecraft");
    let profile_dir = app_data.join("profiles").join(super::safe_segment(&request.profile_id));
    fs::create_dir_all(&profile_dir).map_err(|error| error.to_string())?;
    sync_content(&profile_dir, &request.content)?;

    let java_major = request.java_version.unwrap_or_else(|| super::java_runtime::recommended_major(&request.minecraft_version));
    let java = super::java_runtime::ensure_java(&app, &app_data, java_major)?;
    if let Some(java_bin) = java.parent() {
        let current_path = std::env::var_os("PATH").unwrap_or_default();
        let mut paths = vec![java_bin.to_path_buf()];
        paths.extend(std::env::split_paths(&current_path));
        if let Ok(joined) = std::env::join_paths(paths) { std::env::set_var("PATH", joined); }
    }

    emit_launcher(&app, format!("Minecraft {} 파일 확인 중...", request.minecraft_version));
    ...
    let mut progress = move |event: ProgressEvent| match event {
        ProgressEvent::StageStarted { stage } => emit_launcher(&progress_app, format!("설치 단계: {stage:?}")),
        ProgressEvent::TaskStarted { label, .. } => emit_launcher(&progress_app, format!("다운로드: {label}")),
        _ => {}
    };
    let installed = launcher.install_with_progress(install_request, &mut progress).map_err(|error| error.to_string())?;
    let version = launcher.load_version(&installed.version_id).map_err(|error| error.to_string())?;
    ...
    let account = if request.account.kind == "microsoft" {
        Account::Microsoft {
            ...
            access_token: request.account.minecraft_access_token.ok_or("Minecraft 로그인 토큰이 없습니다. 다시 로그인해 주세요.")?,
        }
    } else {
        Account::offline(request.account.name)
    };
    let command = launcher.build_launch_command_from_version(&version, LaunchOptions {
        account,
    ...
    let mut args = vec![format!("-Xms{}M", request.min_memory_mb), format!("-Xmx{}M", request.max_memory_mb)];
    args.extend(request.java_args);
    args.extend(command.args);
    emit_launcher(&app, format!("{} 실행 중...", installed.version_id));
    let mut process = Command::new(command.executable);
    process.args(args).envs(command.env).current_dir(command.working_dir)
    ...
    if let Some(stdout) = child.stdout.take() { read_game_output(stdout, app.clone()); }
    if let Some(stderr) = child.stderr.take() { read_game_output(stderr, app.clone()); }
    thread::spawn(move || {
...
#[tauri::command]
pub async fn launch_minecraft(app: tauri::AppHandle, request: LaunchMinecraftRequest) -> Result<LaunchStarted, String> {
    if GAME_RUNNING.swap(true, Ordering::SeqCst) { return Err("Minecraft가 이미 실행 중입니다.".into()); }
    let result = match tauri::async_runtime::spawn_blocking(move || prepare_and_launch(app, request)).await {
        Ok(result) => result,
        Err(error) => {
            GAME_RUNNING.store(false, Ordering::SeqCst);
    ...
    if result.is_err() { GAME_RUNNING.store(false, Ordering::SeqCst); }
    result
}
